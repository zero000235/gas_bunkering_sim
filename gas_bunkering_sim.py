...
def render_integrated_report(shared_json_input, rho, mu, epsilon, fluid_key, material, safety_factor, pump_eff, eco_years, eco_hours, eco_elec, eco_carbon_price, install_temp, max_env_temp, min_env_temp, surge_multiplier, eco_ir, widget_key, p_vapor, q_sys_lmin, joint_method='용접 체결 (Welded)', temp_c=20.0, app_mode='bunkering'):
    ...
    try:
        ...
        if is_cryo and total_gas_rate_kg_h > 0:
            ...
            # 비전문가도 쉽게 이해할 수 있는 한글 가공 텍스트 및 스타일 정의
            if x_100_lel_dist <= 1.0:
                flammable_val_text = "1m 미만 (극소 구역)"
                flammable_val_style = "color: #10B981;" # 초록색 (안전)
                risk_summary_text = "가스 누출(기화)량이 급격히 희석되므로, 직접 화재 및 대량 폭발 위험(Methane 5~15% 농도)은 누출 부위 극직근(1m 이내)에만 국한되어 발생 가능성이 거의 없습니다."
            else:
                flammable_val_text = f"풍하측 {x_100_uel_dist:.1f}m ~ {x_100_lel_dist:.1f}m"
                flammable_val_style = "color: #F97316;" # 오렌지색 (위험)
                risk_summary_text = f"풍하측 {x_100_uel_dist:.1f}m ~ {x_100_lel_dist:.1f}m 범위 내에 연소 가능 위험 영역이 발달해 있어, 해당 구역 내 모든 점화원(Ignition Sources)을 차단해야 합니다."

            # HTML 장식 및 복잡한 스타일링을 전면 배제한 순수 마크다운 텍스트 요약본 구성
            report_card_html = f"""
### 💨 BOG & 플래시 가스 실시간 대기 확산 핵심 요약

* **🫧 1. 총 가스 기화량 (Vaporization):**
  **{total_gas_rate_kg_h:.2f} kg/hr** (BOG: {bog_rate_kg_h:.1f} kg/hr | Flash: {flash_rate_kg_h:.1f} kg/hr)
* **🧭 2. 가스 확산 방향 (Wind & Spread):**
  **{spread_dir_kor} 방향** (실시간 풍향 {current_wind_dir:.0f}°, 풍속 {current_wind_spd:.1f}m/s)
* **🔥 3. 연소 가능 범위 (LEL):**
  **{flammable_val_text}** (메탄 농도 5% ~ 15% 가연 영역)
* **🚨 4. 안전 대피 권고 반경 (Exclusion Zone):**
  **{x_20_lel_dist * 1.2:.1f} m** (가스가 미량 잔존할 가능성까지 감안해 20%의 안전 계수를 추가 적용한 최종 대피 한계 반경)

---

⚠️ **현장 안전 조치 및 대피 반경 권고:**
{risk_summary_text} 기상 조건({stability_desc} 상태, 풍속 {current_wind_spd:.1f}m/s)과 가스 희석 거동을 감안하여, **예방 차원의 비상 대피 및 접근 통제 반경을 누설원 중심 {x_20_lel_dist * 1.2:.1f}m로 권고**합니다.
"""
        else:
            report_card_html = """
### 💨 BOG & 플래시 가스 실시간 대기 확산 분석
* **상태:** 상온 유체(Non-Cryogenic Fluid) 해석 중
* **안내:** BOG 및 플래시 가스 실시간 확산 모델은 **극저온 액체(LNG/LPG) 벙커링 모드**에서만 활성화됩니다.
"""

        # 대시보드를 탭으로 분리하여 정보 피로도 감소 및 가시성 극대화
        # --- [🫧 펌프 공동현상(Cavitation) 및 NPSH 안전 정밀 진단 연산] ---
        pump_node_id = None
        for n in nodes_list:
            if n["type"] == "pump":
                pump_node_id = n["id"]
        ...
        if is_cryo:
            ...
            bog_card_html = f"""
            <div style='background: rgba(15, 23, 42, 0.6); padding: 1.3rem; border-radius: 14px; border: 1.2px solid rgba(148, 163, 184, 0.15); text-align: center; box-shadow: inset 0 0 12px rgba(148, 163, 184, 0.02);'>
                <span style='font-size: 0.82rem; color: #94A3B8; display: block; margin-bottom: 0.5rem; font-weight: 600;'>🫧 5. 극저온 BOG 발생률</span>
                <strong style='font-size: 1.4rem; color: #94A3B8; font-family: "Outfit", sans-serif;'>N/A (상온 유체)</strong>
                <div style='font-size: 0.76rem; color: #64748B; margin-top: 0.6rem; border-top: 1px dashed rgba(148, 163, 184, 0.1); padding-top: 0.5rem; line-height: 1.45;'>
                    ▪ 상온 유체 해석 모드<br>
                    ▪ 비기화 단열 상태
                </div>
            </div>
        ...
        # 대시보드를 탭으로 분리하여 정보 피로도 감소 및 가시성 극대화
        tab_hydraulic, tab_dispersion = st.tabs(["📊 배관 수력학 및 LCC 종합 진단", "💨 대기 확산 거동 분석"])
        ...
        with tab_dispersion:
            # BOG & 플래시 가스 실시간 거동 대기 확산 보고서 추가 렌더링
            st.markdown(report_card_html, unsafe_allow_html=True)# --- 🌟 [3대 전문 엔지니어링 실무 검토서 (자중 처짐/기밀 위험/임계 유속)] 🌟 ---
