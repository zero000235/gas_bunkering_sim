...
def render_integrated_report(shared_json_input, rho, mu, epsilon, fluid_key, material, safety_factor, pump_eff, eco_years, eco_hours, eco_elec, eco_carbon_price, install_temp, max_env_temp, min_env_temp, surge_multiplier, eco_ir, widget_key, p_vapor, q_sys_lmin, joint_method='용접 체결 (Welded)', temp_c=20.0, app_mode='bunkering'):
    ...
    try:
        ...
        if is_cryo and total_gas_rate_kg_h > 0:
            ...
            report_card_html = f"""
            <div style='background: rgba(30, 41, 59, 0.65); padding: 1.8rem; border-radius: 18px; border: 1px solid rgba(249, 115, 22, 0.35); box-shadow: 0 15px 30px rgba(0, 0, 0, 0.4); margin-top: 1.5rem; margin-bottom: 2rem; backdrop-filter: blur(10px);'>
                <h4 style='margin-top: 0; color: #F97316; font-family: "Outfit", sans-serif; font-weight: 800; font-size:1.3rem; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom:0.6rem; display: flex; align-items: center; gap: 8px;'>💨 BOG & 플래시 가스(Flash Gas) 실시간 대기 확산 거동 분석서</h4>
                
                <div style='display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.2rem; margin-top: 1.2rem;'>
                    <!-- 지속 발생량 정보 -->
                    <div style='background: rgba(15, 23, 42, 0.5); padding: 1.2rem; border-radius: 12px; border: 1px solid rgba(56, 189, 248, 0.25); text-align: left;'>
                        <span style='font-size: 0.85rem; color: #94A3B8; display: block; margin-bottom: 0.4rem;'>🫧 총 가스 기화량 (BOG + Flash)</span>
                        <strong style='font-size: 1.25rem; color: #38BDF8;'>{total_gas_rate_kg_h:.2f} kg/hr</strong>
                        <div style='font-size: 0.78rem; color: #64748B; margin-top: 0.4rem; line-height: 1.5;'>
                            ▪ 자연 증발 (BOG): {bog_rate_kg_h:.2f} kg/hr<br>
                            ▪ 감압 기화 (Flash): {flash_rate_kg_h:.2f} kg/hr
                        </div>
                    </div>
                    
                    <!-- 바람 기상 정보 -->
                    <div style='background: rgba(15, 23, 42, 0.5); padding: 1.2rem; border-radius: 12px; border: 1px solid rgba(251, 191, 36, 0.25); text-align: left;'>
                        <span style='font-size: 0.85rem; color: #94A3B8; display: block; margin-bottom: 0.4rem;'>🧭 실시간 기상 및 가스 흐름</span>
                        <strong style='font-size: 1.25rem; color: #FBBF24;'>{current_wind_spd:.1f} m/s | {spread_dir_kor}</strong>
                        <div style='font-size: 0.78rem; color: #64748B; margin-top: 0.4rem; line-height: 1.5;'>
                            ▪ 실시간 풍향: {current_wind_dir:.1f}°<br>
                            ▪ 가스 확산 방향: {spread_dir_deg:.1f}° ({spread_dir_kor} 방향)
                        </div>
                    </div>
                    
                    <!-- 가연성 구역 거리 정보 -->
                    <div style='background: rgba(15, 23, 42, 0.5); padding: 1.2rem; border-radius: 12px; border: 1px solid rgba(249, 115, 22, 0.25); text-align: left;'>
                        <span style='font-size: 0.85rem; color: #94A3B8; display: block; margin-bottom: 0.4rem;'>🔥 연소 가능 위험 구역 (Flammable)</span>
                        <strong style='font-size: 1.25rem; color: #F97316;'>{x_100_uel_dist:.1f}m ~ {x_100_lel_dist:.1f}m</strong>
                        <div style='font-size: 0.78rem; color: #64748B; margin-top: 0.4rem; line-height: 1.5;'>
                            ▪ 사전경고(20% LEL) 한계: {x_20_lel_dist:.1f} m<br>
                            ▪ 산소결핍(15% UEL) 코어: {x_100_uel_dist:.1f} m 이내
                        </div>
                    </div>
                </div>
                
                <div style='margin-top: 1.2rem; background: rgba(239, 68, 68, 0.08); border: 1px solid rgba(239, 68, 68, 0.25); padding: 1rem; border-radius: 10px; font-size: 0.85rem; color: #FDA4AF; line-height: 1.6;'>
                    ⚠️ <b>물리 거동 위험 분석 및 안전 격리 권고:</b><br>
                    현재 대기 안정도는 <b>{stability_desc}</b> 상태로, 풍속 {current_wind_spd:.1f}m/s 조건 하에서 발생한 가스가 <b>{spread_dir_kor}</b> 방향으로 확산 중입니다.<br>
                    연소/폭발이 가능한 메탄 농도 5%~15% 영역이 풍하측 <b>{x_100_uel_dist:.1f}m ~ {x_100_lel_dist:.1f}m</b> 사이에 기포 형태로 체류하고 있으니, 해당 구간 내 모든 점화원(Ignition Sources)을 긴급 차단하고 <b>최소 {x_20_lel_dist * 1.2:.1f}m(안전율 20% 가산)</b>의 안전 격리 거리(Safety Exclusion Zone)를 유지하십시오.
                </div>
            </div>
            """
        else:
            report_card_html = f"""
            <div style='background: rgba(30, 41, 59, 0.65); padding: 1.8rem; border-radius: 18px; border: 1px solid rgba(148, 163, 184, 0.15); box-shadow: 0 15px 30px rgba(0, 0, 0, 0.4); margin-top: 1.5rem; margin-bottom: 2rem; backdrop-filter: blur(10px);'>
                <h4 style='margin-top: 0; color: #94A3B8; font-family: "Outfit", sans-serif; font-weight: 800; font-size:1.3rem; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom:0.6rem;'>💨 BOG & 플래시 가스 실시간 대기 확산 거동 분석서</h4>
                <p style='color: #94A3B8; font-size: 0.9rem; text-align: center; margin: 2rem 0;'>
                    상온 유체(Non-Cryogenic Fluid)가 해석 중입니다. BOG 및 플래시 가스 거동 분석은 <b>극저온 액체(LNG/LPG) 벙커링 모드</b>에서만 제공됩니다.
                </p>
            </div>
            """
        ...
        if is_cryo:
            bog_card_html = f"""
                <div style='background: rgba(15, 23, 42, 0.5); padding: 1.2rem; border-radius: 12px; border: 1px solid rgba(56, 189, 248, 0.35); text-align: center;'>
                    <span style='font-size: 0.85rem; color: #94A3B8; display: block; margin-bottom: 0.4rem;'>🫧 5. 극저온 BOG 발생률 ({fluid_key})</span>
                    <strong style='font-size: 1.25rem; color: #38BDF8;'>{bog_percent:.3f} % / hr</strong>
                    <span style='font-size: 0.78rem; color: #64748B; display: block; margin-top: 0.4rem;'>(증발가스: {bog_rate_kg_h:.2f} kg/hr | 침입열: {total_heat_leak_watts:.1f} W)</span>
                </div>
            """
        else:
            bog_card_html = f"""
                <div style='background: rgba(15, 23, 42, 0.5); padding: 1.2rem; border-radius: 12px; border: 1px solid rgba(148, 163, 184, 0.15); text-align: center;'>
                    <span style='font-size: 0.85rem; color: #94A3B8; display: block; margin-bottom: 0.4rem;'>🫧 5. 극저온 BOG 발생률</span>
                    <strong style='font-size: 1.25rem; color: #94A3B8;'>N/A (상온 유체)</strong>
                    <span style='font-size: 0.78rem; color: #64748B; display: block; margin-top: 0.4rem;'>(상온 유체 해석 모드로 비기화 상태)</span>
                </div>
            """
    ...
    with main_tabs[0]:
        ...
        <body>
            ...
            <script>
                ...
                const windAutoRotate = ("WIND_AUTO_ROTATE_PLACEHOLDER" === "true");
        ...
        # 기상 수동 조절 슬라이더 및 자동 스위핑 옵션
        col_wind1, col_wind2, col_wind3 = st.columns([2, 2, 1.5])
        with col_wind1:
        ...
        with col_wind3:
            st.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True) # 레이아웃 정렬용 공백
            wind_auto_rotate = st.toggle(
                "🧭 풍향 자동 회전",
                value=False,
                key="wind_auto_rotate_t1",
                help="활성화 시, 브라우저 상에서 바람 풍향이 360도로 부드럽게 자동 회전하며 연소 구역의 실시간 변화를 주기적으로 보여줍니다."
            )
        ...
        rendered_canvas_html = canvas_html.replace(
            ...
            "WIND_AUTO_ROTATE_PLACEHOLDER", "true" if wind_auto_rotate else "false"
        ).replace(
